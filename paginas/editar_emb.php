...
<head>
  ...
  function redirectToListagem() {
    ...
    }, 2000); // Ajuste o tempo de redirecionamento
  }
  </script>
  <style>
...
<body>
  <div class="login-container">
    <img src="../imagens/SISMARINAS.png" alt="Logo da Marinha">
    <h2>Alteração de Embarcação</h2>

    <form method="POST">
      <label for="nome">Nome da Embarcação:</label>
      <input type="text" id="nome" name="nome"
        value="<?php echo isset($embarcacao['nome']) ? $embarcacao['nome'] : ''; ?>" required>

      <label for="numero_serie">Número de Série:</label>
      <input type="text" id="numero_serie" name="numero_serie"
        value="<?php echo isset($embarcacao['numero_serie']) ? $embarcacao['numero_serie'] : ''; ?>" required>

      <label for="tipo">Tipo:</label>
      <input type="text" id="tipo" name="tipo"
        value="<?php echo isset($embarcacao['tipo']) ? $embarcacao['tipo'] : ''; ?>" required>

      <label for="observacao">Observação:</label>
      <textarea id="observacao" name="observacao"
        required><?php echo isset($embarcacao['observacao']) ? $embarcacao['observacao'] : ''; ?></textarea>

      <label for="proporcao_motor">Proporção do Motor:</label>
      <input type="text" id="proporcao_motor" name="proporcao_motor"
        value="<?php echo isset($embarcacao['proporcao_motor']) ? $embarcacao['proporcao_motor'] : ''; ?>" required>

      <label for="dt_validade">Data de Validade:</label>
      <input type="date" id="dt_validade" name="dt_validade"
        value="<?php echo isset($embarcacao['dt_validade']) ? $embarcacao['dt_validade'] : ''; ?>" required>

      <label for="status">Status Legal:</label>
      <select id="status" name="status" required>
        <option value="LEGAL"
          <?php echo (isset($embarcacao['status']) && $embarcacao['status'] === 'LEGAL') ? 'selected' : ''; ?>>LEGAL
        </option>
        <option value="ILEGAL"
          <?php echo (isset($embarcacao['status']) && $embarcacao['status'] === 'ILEGAL') ? 'selected' : ''; ?>>ILEGAL
        </option>
      </select>
